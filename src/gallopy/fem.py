...
class FEMSolver1D(object):
    def __init__(self,
                 ...
                 force_func: Union[Callable[[float], float], float],
...
class FEMSolver2D(object):
    def __init__(self,
                 ...
                 force_func: Union[Callable[[float, float], float], float],
                 boundary_conditions: Sequence[BoundaryCondition]):
    ...
    def _cal_param_arr(self,
        ...
        if isinstance(self.force_func, Callable):
            f = self.force_func(x_arr, y_arr)  # TODO: 应为单元的 f 值
        else:
            f = np.ones(segments_num) * self.force_func
    ...
    def solve(self, triangulation: Triangulation):
        ...
        a_arr_3xN, b_arr_3xN, c_arr_3xN = self._cal_abc_arr_3xN(ns_mat, x_arr, y_arr)
    ...
    def _cal_abc_arr_3xN(self, ns_mat, x_arr, y_arr):
        arr_len = len(ns_mat)
        ...
        a_arr_3xN[0] = x2_arr*y3_arr - y2_arr*x3_arr
        a_arr_3xN[1] = x3_arr*y1_arr - y3_arr*x1_arr
        a_arr_3xN[2] = x1_arr*y2_arr - y1_arr*x2_arr
        
        b_arr_3xN[0] = y2_arr - y3_arr
        b_arr_3xN[1] = y3_arr - y1_arr
        b_arr_3xN[2] = y1_arr - y2_arr
        
        c_arr_3xN[0] = x3_arr - x2_arr
        c_arr_3xN[1] = x1_arr - x3_arr
        c_arr_3xN[2] = x2_arr - x1_arr
    ...
    def _cal_b_mat_arr_3xN(self, Delta_arr, f):
        b_mat_arr = np.zeros((3, len(Delta_arr)))
